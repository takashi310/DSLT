...
__global__ void convolutionRowsKernel8(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
	...
	if((baseY < imageH) && (baseZ < imageZ)){
		...
		for(int i = ROWS_HALO_STEPS; i < ROWS_HALO_STEPS + ROWS_RESULT_STEPS; i++){
			...
			for(int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
				sum += c_Kernel8[KERNEL_RADIUS - j] * s_Data[threadIdx.z][threadIdx.y][threadIdx.x + i * ROWS_BLOCKDIM_X + j];
...
extern "C" __declspec (dllexport) void convolutionRowsGPU8(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
    ...
    convolutionRowsKernel8<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
...
__global__ void convolutionColumnsKernel8(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
	...
	if((baseX < imageW) && (baseZ < imageZ)){
		...
		for(int i = COLUMNS_HALO_STEPS; i < COLUMNS_HALO_STEPS + COLUMNS_RESULT_STEPS; i++){
			...
			//#pragma unroll
			for(int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
				sum += c_Kernel8[KERNEL_RADIUS - j] * s_Data[threadIdx.z][threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y + j];
...
extern "C" __declspec (dllexport) void convolutionColumnsGPU8(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
...
//	assert( imageZ % COLUMNS_BLOCKDIM_Z == 0 );
    ...
    convolutionColumnsKernel8<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
...
__global__ void convolutionZColumnsKernel8(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
	...
	if((baseX < imageW) && (baseY < imageH)){
		...
		for(int i = Z_COLUMNS_HALO_STEPS; i < Z_COLUMNS_HALO_STEPS + Z_COLUMNS_RESULT_STEPS; i++){
			...
			//#pragma unroll
			for(int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
				sum += c_Kernel8[KERNEL_RADIUS - j] * s_Data[threadIdx.x][threadIdx.y][threadIdx.z + i * Z_COLUMNS_BLOCKDIM_Z + j];
...
extern "C" __declspec (dllexport) void convolutionZColumnsGPU8(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
    ...
    convolutionZColumnsKernel8<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
