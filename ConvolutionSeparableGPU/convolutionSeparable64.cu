...
#define   Z_COLUMNS_BLOCKDIM_Y 2
#define   Z_COLUMNS_BLOCKDIM_Z 16
...
////////////////////////////////////////////////////////////////////////////////
__constant__ float c_Kernel64[KERNEL_LENGTH];

extern "C" __declspec (dllexport) void setConvolutionKernel64(float *h_Kernel, int kernel_radius){
	...
	cudaMemcpyToSymbol(c_Kernel64, kernel, KERNEL_LENGTH * sizeof(float));
}
...
__global__ void convolutionRowsKernel64(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
	...
	if((baseY < imageH) && (baseZ < imageZ)){
		...
		for(int i = ROWS_HALO_STEPS; i < ROWS_HALO_STEPS + ROWS_RESULT_STEPS; i++){
			...
			for(int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
				sum += c_Kernel64[KERNEL_RADIUS - j] * s_Data[threadIdx.z][threadIdx.y][threadIdx.x + i * ROWS_BLOCKDIM_X + j];
...
extern "C" __declspec (dllexport) void convolutionRowsGPU64(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
    ...
    convolutionRowsKernel64<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
...
__global__ void convolutionColumnsKernel64(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
	...
	if((baseX < imageW) && (baseZ < imageZ)){
		...
		for(int i = COLUMNS_HALO_STEPS; i < COLUMNS_HALO_STEPS + COLUMNS_RESULT_STEPS; i++){
			...
			//#pragma unroll
			for(int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
				sum += c_Kernel64[KERNEL_RADIUS - j] * s_Data[threadIdx.z][threadIdx.x][threadIdx.y + i * COLUMNS_BLOCKDIM_Y + j];
...
extern "C" __declspec (dllexport) void convolutionColumnsGPU64(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
...
//	assert( imageZ % COLUMNS_BLOCKDIM_Z == 0 );
    ...
    convolutionColumnsKernel64<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
...
__global__ void convolutionZColumnsKernel64(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
	...
	if((baseX < imageW) && (baseY < imageH)){
		...
		for(int i = Z_COLUMNS_HALO_STEPS; i < Z_COLUMNS_HALO_STEPS + Z_COLUMNS_RESULT_STEPS; i++){
			...
			//#pragma unroll
			for(int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++)
				sum += c_Kernel64[KERNEL_RADIUS - j] * s_Data[threadIdx.x][threadIdx.y][threadIdx.z + i * Z_COLUMNS_BLOCKDIM_Z + j];
...
extern "C" __declspec (dllexport) void convolutionZColumnsGPU64(
    float *d_Dst,
    float *d_Src,
    int imageW,
    int imageH,
    ...
    convolutionZColumnsKernel64<<<blocks, threads>>>(
        d_Dst,
        d_Src,
        imageW,
        imageH,
